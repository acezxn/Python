...
class client_crypto:
    ...
    def decrypt(self, msg): # decrypt communication
        msg = base64.b64decode(msg)import socket
...
def Print_response(s, crypto, colors): # client side response printer (background)
            ...
            if data.strip() != 'Insert password' and 'Access Granted' not in data:
                if len(data.strip()) > 0:
                    if data.split(': ')[0] == 'Info':
                        print(colors['OKBLUE']+ data.split(': ')[0]+ ': '+colors['GREEN']+': '.join(data.split(': ')[1:]))
                    elif data.split(': ')[0] == 'Warning':
                        print(colors['RED']+ data.split(': ')[0]+ ': '+colors['GREEN']+': '.join(data.split(': ')[1:]))
                    else:
                        print(colors['YELLOW']+ data.split(': ')[0]+ ': '+colors['GREEN']+': '.join(data.split(': ')[1:]))
...
def connecting(name, crypto, colors): # try to connect and start printing and accepting response
    ...
    try:
        s.connect((ip, port))
        start_new_thread(Print_response, (s, crypto, colors))
        out = input()
        ...
        out = crypto.encrypt(name.encode('utf-8'))
        time.sleep(0.1)
        s.send(out)
        while True:
            out = '\n'+name + '@' + lhost + ': '+input()+' \n'
            print(colors['YELLOW']+ out.split(': ')[0]+ ': '+colors['GREEN']+': '.join(out.split(': ')[1:]))
            out = crypto.encrypt(out.encode('utf-8'))
            s.send(out)
    except Exception as e:
        print(e)
